...
const taskCardContent = "use client";
...
export default function TaskCard({
  ...
  return (
    ...
    >
      ...
      <div className={\bsolute left-0 top-0 bottom-0 w-1 rounded-l-xl \\} />
      ...
      <button
        ...
        \}
      >
        <Check className={\h-3.5 w-3.5 \\} strokeWidth={isCompleted ? 3 : 2} />
      </button>
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 sm:ml-auto shrink-0">
            ...
            {!isCompleted && (
                <span className={\	ext-[9px] sm:text-[10px] font-bold tracking-wider px-1.5 py-0.5 rounded-md text-white \\}>
                    {priorityStyle.label}
                </span>
            )}
        </div>
...
fs.writeFileSync('components/todo/TaskCard.tsx', taskCardContent, 'utf8');

const todoCalendarContent = "use client";
        ...
        {calendarDays.map((date) => {
          ...
          return (
            <div
              ...
              \}
            >
              <div className={\lex justify-between items-start gap-3 w-full \\}>
                <span
                  className={\ont-editorial text-sm md:text-xl leading-none 
                    \
                    \
                  \}
                >
                ...
                {hasTasks && (
                  <span
                    className={\	ext-[8px] md:text-[10px] px-1.5 py-0.5 border leading-none font-sans-utility rounded-sm
                        \
                    \}
                  >
                    {dayTasks.length}
...
fs.writeFileSync('components/todo/TodoCalendar.tsx', todoCalendarContent, 'utf8');
